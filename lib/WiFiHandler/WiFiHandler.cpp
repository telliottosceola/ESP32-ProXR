...
bool WiFiHandler::checkWiFi(bool setupMode){
  ...
  if(!WiFi.isConnected()){
    ...
    if(WiFi.status() != WL_CONNECTED){
      ...
      String dnsName = "NCD-"+macAddress.substring(12);
      int index = dnsName.indexOf(':');
      dnsName.remove(index,1);
      char dnsChar[dnsName.length()+1];
      dnsName.toCharArray(dnsChar, sizeof(dnsChar));
      if(!MDNS.begin(dnsChar)) {
        Serial.println("Error starting mDNS");
      }
      Serial.printf("DNS URL: %s.local\n",dnsChar);
      #ifdef DEBUG
...
void WiFiHandler::scanNetworks(){
  ...
  if(n < 1){
    ...
    for(int i = 0; i < n; i++){
      ...
      char apMacChar[18];
      char* macFormat = (char*)"%02X:%02X:%02X:%02X:%02X:%02X";
